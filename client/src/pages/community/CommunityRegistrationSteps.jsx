...
import {
  Plus, Trash2, Upload, X, User, Phone,
  Briefcase, Image, CreditCard, Hash, Link2, Loader2,
} from "lucide-react";
import {
  OFFERING_CATEGORIES, EVENT_SUBTYPES, MEMBER_ROLES, ID_TYPES,
...
// ─── STEP 2 — Team Members + per-member ID details ───────────
const EMPTY_MEMBER = {
  full_name: "", phone: "", role: "", is_owner: false,
  id_type: "", id_number: "",
  id_image_url: "", id_image_public_id: "", id_link: "",
};

const memberInputCls =
  "w-full rounded-xl border border-[#E0D8CE] bg-white pl-8 pr-3 py-2.5 text-sm text-[#1A2820] focus:outline-none focus:border-[#3E7A58] focus:ring-2 focus:ring-[#3E7A58]/10 transition-all placeholder:text-[#C4B8A8]";

export function Step2TeamAndDocs({ members, setMembers, errors }) {
  const fileRefs = useRef({});
  const [uploading, setUploading] = useState({}); // { [idx]: bool }
  const [uploadErr, setUploadErr] = useState({}); // { [idx]: string }
  ...
  const handleIdImage = async (i, file) => {
    if (!file) return;
    setUploadErr((e) => ({ ...e, [i]: "" }));
    setUploading((u) => ({ ...u, [i]: true }));
    try {
      const res = await uploadService.uploadImage(file, UPLOAD_FOLDERS.DOCUMENT);
      const url = res?.data?.url;
      const publicId = res?.data?.public_id;
      if (!url) throw new Error("Upload did not return a URL");
      setMembers((m) => m.map((mem, idx) =>
        idx === i ? { ...mem, id_image_url: url, id_image_public_id: publicId || "" } : mem));
    } catch (err) {
      setUploadErr((e) => ({ ...e, [i]: err.message || "Image upload failed" }));
    } finally {
      setUploading((u) => ({ ...u, [i]: false }));
    }
  };

  const clearIdImage = (i) =>
    setMembers((m) => m.map((mem, idx) =>
      idx === i ? { ...mem, id_image_url: "", id_image_public_id: "" } : mem));
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-sm font-semibold text-[#1A2820]">Team Members & ID Verification *</h3>
            <p className="text-xs text-[#9A9285] mt-0.5">Add each member with their government ID details so our security team can verify them.</p>
          </div>
          <button type="button" onClick={addMember}
            className="flex items-center gap-1.5 rounded-xl bg-[#1C3D2E] px-3 py-2 text-xs font-semibold text-white hover:opacity-90 transition-all shrink-0">
          ...
          {members.map((m, i) => {
            ...
            return (
              <div key={i} className="rounded-2xl border border-[#E8E1D5] bg-[#FAFAF8] p-4 space-y-3">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input value={m.full_name} onChange={(e) => updateMember(i, "full_name", e.target.value)}
                        placeholder="Full name" className={memberInputCls} />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input value={m.phone} onChange={(e) => updateMember(i, "phone", e.target.value)}
                        placeholder="+91 98765 43210" className={memberInputCls} />
                    </div>
                  </div>
                </div>
                ...
                {/* ── ID verification ── */}
                <div className="rounded-xl border border-[#E0D8CE] bg-white p-3 space-y-3">
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-[#9A9285]">ID Verification</p>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <Label>ID Type *</Label>
                      <div className="relative">
                        <CreditCard size={14} className="absolute left-3 top-3 text-[#B8AFA4] pointer-events-none z-10" />
                        <select value={m.id_type || ""} onChange={(e) => updateMember(i, "id_type", e.target.value)}
                          className="w-full rounded-xl border border-[#E0D8CE] bg-white pl-8 pr-3 py-2.5 text-sm text-[#1A2820] focus:outline-none focus:border-[#3E7A58] transition-all appearance-none">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <Hash size={14} className="absolute left-3 top-3 text-[#B8AFA4] pointer-events-none" />
                        <input value={m.id_number || ""} onChange={(e) => updateMember(i, "id_number", e.target.value)}
                          placeholder="e.g. ABCDE1234F" className={memberInputCls} />
                      </div>
                    </div>
                  </div>

                  <div>
                    <Label>ID Proof * — upload a photo or paste a link</Label>
                    <div className="flex flex-col gap-2">
                      {hasImage ? (
                        <div className="flex items-center gap-3 rounded-xl border border-[#3E7A58] bg-[#EEF5F1] px-3 py-2">
                          <img src={m.id_image_url} alt="ID proof" className="h-10 w-10 rounded object-cover border border-[#D4C9BB]" />
                          <a href={m.id_image_url} target="_blank" rel="noreferrer"
                            className="flex-1 text-xs font-medium text-[#1C3D2E] hover:underline truncate">ID image uploaded — view</a>
                          <button type="button" onClick={() => clearIdImage(i)} className="text-red-500 hover:text-red-700 transition-colors">
                            <X size={14} />
                          </button>
                        </div>
                      ) : (
                        <button type="button" disabled={isUploading} onClick={() => fileRefs.current[i]?.click()}
                          className="flex items-center justify-center gap-2 rounded-xl border-2 border-dashed border-[#D4C9BB] bg-[#F5F2EE] px-3 py-3 text-sm font-medium text-[#6B7C6E] hover:border-[#3E7A58] hover:bg-[#EEF5F1] transition-all disabled:opacity-60">
                          {isUploading
                            ? <><Loader2 size={16} className="animate-spin" /> Uploading…</>
                            : <><Upload size={16} /> Upload ID photo (JPG/PNG)</>}
                        </button>
                      )}
                      <input ref={(el) => (fileRefs.current[i] = el)} type="file"
                        accept=".jpg,.jpeg,.png,image/jpeg,image/png" className="hidden"
                        onChange={(e) => { handleIdImage(i, e.target.files?.[0]); e.target.value = ""; }} />

                      <div className="relative">
                        <Link2 size={14} className="absolute left-3 top-3 text-[#B8AFA4] pointer-events-none" />
                        <input value={m.id_link || ""} onChange={(e) => updateMember(i, "id_link", e.target.value)}
                          placeholder="…or paste a document link (Google Drive, etc.)" className={memberInputCls} />
                      </div>

                      {uploadErr[i] && <p className="text-xs text-red-500">{uploadErr[i]}</p>}
                    </div>
                  </div>
                </div>
              </div>
